...
def document():
    wb=Workbook()
    wb.create_sheet('111',index=0)
    worksheet=wb['111']
    if start==1:
        title_list=['序号','存币','目标币','存币数量','测试汇率','旧系统汇率','误差']
        for i in range(7):
    ...
    for i in range(num):
        ...
        for k in range(num):
            ...
            # print('目标币:',upper_rec)
                ...
                exchangeRate()
                # try:
                diff=float((rate-realInstantRate)/realInstantRate)
                print(start,upper_dep,upper_rec,'存币数量:',exchangeAmount,'测试汇率:',rate,'生产汇率:',realInstantRate,'误差:',diff)
                currentlist=[start,upper_dep,upper_rec,exchangeAmount,rate,realInstantRate,diff]
                if start==1:
                    title_list=['序号','存币','目标币','存币数量','测试汇率','旧系统汇率','误差']
                    for i in range(7):
                        worksheet.cell(row=1,column=i+1,value=title_list[i])
